...
def generateSubsquares(path):
    ...
    # If image is grayscale (2D), manually create YUV. Otherwise, convert from RGB.
    if img.ndim == 2:
        # Grayscale image is the Y channel
        y_channel = img
        # U and V channels are just zeros
        u_channel = np.zeros_like(img)
        v_channel = np.zeros_like(img)
        # Stack them to create a 3D YUV image
        yuv = np.stack([y_channel, u_channel, v_channel], axis=-1)
    else:
        # For color images, perform the standard conversion
        yuv = retrieveYUV(img)
